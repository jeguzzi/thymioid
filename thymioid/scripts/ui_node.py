...
class WifiUI(object):
    ...
    def button_up(self, button):
        if(button == 'left'):
            ...
            self.desired_configuration = (
                self.desired_configuration + 1) % len(self.configurations)
            self.update_leds()
    ...
    def init_configuration(self):
        ...
        self.configurations = [(None, None, False),
                               ('wlan0', '192.168.168.1', True)]

        additional_interfaces = rospy.get_param("~wlan_interfaces", [])

        for interface in additional_interfaces:
            config = (interface.get('name'), interface.get(
                'ip', ''), interface.get('is_ap', False))
    ...
    def check_ap(self):
        ...
        # return self.service_is_running("dnsmasq") and
        # self.service_is_running("hostapd")
    ...
    def stop_ap(self):
        # iface should start/stop automatically dhcp server and hostapd (see
        # /etc/network/interfaces)

        try:
            rospy.loginfo(subprocess.check_output(
                ["sudo", "/etc/init.d/dnsmasq", "stop"]))
            rospy.loginfo(subprocess.check_output(
                ["sudo", "/etc/init.d/hostapd", "stop"]))
        except Exception as e:
            rospy.logerr(
                "While stopping dnsmaq and hostpad, got exception %s" % e)
